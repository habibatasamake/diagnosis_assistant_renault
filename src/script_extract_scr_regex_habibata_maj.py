...
class TreeNodeV2:
    ...
    def _to_rows(self, url=None, equipment=None, page=None, parent_symptom=""):
        """
        Fonction auxiliaire qui parcourt récursivement le nœud et ses descendants
        pour construire une liste de dictionnaires contenant les informations SCR.
        
        :param url: (optionnel) chaîne indiquant l'URL ou le nom de fichier du document.
        :param equipment: (optionnel) nom de l'équipement.
        :param page: (optionnel) numéro de page du document.
        :param parent_symptom: (interne) héritage du symptôme du parent si le nœud n'a pas de contenu.
        :return: liste de dictionnaires.
        """
        rows = []
        # Utilise le contenu du nœud s'il existe, sinon hérite du symptôme du parent
        current_symptom = self.content.strip() if self.content.strip() else parent_symptom
        if current_symptom:  # Si un symptôme est présent
            rows.append({
    ...
    def to_dataframe(self, url=None, equipment=None, page=None):
        """
        Convertit le nœud (et tous ses descendants) en un DataFrame structuré.
        Les colonnes retournées sont :
        - URL: Le nom du document ou son URL,
        - equipment: Le nom de l'équipement,
        - page: Le numéro de la page (si applicable),
        - symptom: Le contenu du nœud (correspondant au symptôme),
        - cause: La cause extraite (s'il existe, sinon "Unknown"),
        - remedy: Le remède extrait (s'il existe, sinon "Unknown").

        :param url: (optionnel) chaîne indiquant l'URL ou le nom du document.
        :param equipment: (optionnel) nom de l'équipement.
        :param page: (optionnel) numéro de la page.
        :return: un DataFrame pandas contenant une ligne par nœud (et descendant) avec les informations SCR.
        """
